...
{
    [Info("Human NPC", "Ts3Hosting", "0.3.50")]
...
//                if (IsInvoking("AllowMove")) { CancelInvoke("AllowMove"); AllowMove(); }
        ...
        private List<BasePlayer> ListAllNPC()
        {
            List<BasePlayer> npcspawned = new List<BasePlayer>();
            foreach (KeyValuePair<ulong, HumanNPCInfo> pair in humannpcs)
            {
                if (!pair.Value.enable) continue;
                npcspawned.Add(FindPlayerByID(pair.Key));
            }
            return npcspawned;
        }
